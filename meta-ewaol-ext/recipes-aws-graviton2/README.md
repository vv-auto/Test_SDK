# Arm-based AWS Graviton 2 Instances
...
1. The resources created by deploying the [VMImport CloudFormation Template](meta-ewaol-ext/recipes-aws-graviton2/cfn/vmimport-cfn.yml) (roles, policies, S3 bucket for images).
...
1. Clone the repo from the instance or upload the code and invoke build command. For example:
    ...
    ```bash
    git clone https://gitlab.com/soafee/ewaol/meta-ewaol-machine.git
...
1. Customize the ewaol-graviton2-ami.yaml as needed and invoke build.
    ...
    ```bash
    kas build kas/machine/ewaol-graviton2-ami.yaml
    ```
